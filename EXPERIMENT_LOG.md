...
## 2026-09-07T14:37:51 — Reset after reconnect

User authorized resetting stale stack. Stopped 31 owned processes; no survivors. Saved floor1 map checksums verified unchanged. Restarted inputs only via inputs.sh up (109 s). Odometry ~50 Hz, scans 10 Hz, IMU ~100 Hz, camera metadata 30 Hz with fresh RGB and aligned depth samples; measured stow 100%, estop clear, commands zero. No SLAM/Nav2 remain. Audit hardware_ok true, no scan-rate warnings: captures/input_checks/20260907T183531Z/inputs.json. Overall startup exit 1 because arm_tool reports zero offset and load rather than its expected 172 mm/0.82 kg. No calibration changes, authority requests, motion or localization performed. Destination for user phrase "reach it" remains unspecified; treated as hardware reconnection.
